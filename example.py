...
while True:
    # convert prompt to PDDL
    pddl_domain = model.convert(domain_prompt)
    ...
    user_input = input("Enter 'exit' to finish, or provide instructions: ")
    if user_input == 'exit':
        break
    else:
        # domain_prompt += f'\n{user_input}' + feedback 
        model.refine(pddl_domain, feedback, user_input=user_input)
