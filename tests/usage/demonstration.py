...
def run_aba_alg(
    model: LLM, action_model, domain_desc, hierarchy, prompt, max_iter: int = 2
    ...
    for _ in range(max_iter):
        ...
        for _, action in enumerate(actions):
            ...
            pddl_action, new_preds, response = builder.extract_pddl_action(
                model=model,
...
def run_aba():
    ...
    hier = load_file(f"{base_path}hierarchy_requirements.json")
